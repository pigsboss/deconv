...
o_mpmean=zeros(npixels,1); % mean of most-probable interval.
o_mpmedian=zeros(npixels,1); % median of most-probable interval.
...
%    [o_mpll(k),o_mpul(k),prob_ints(k,:)]=mostprob(O_src(k,:),nbins);
    [o_mpll(k),o_mpul(k),o_mpmean(k),o_mpmedian(k),~]=mostprob(O_src(k,:),nbins);
...
if length(dim)>2
    ...
    o_mpmean=reshape(o_mpmean,dim(1),[]);
    o_mpmedian=reshape(o_mpmedian,dim(1),[]);
    prob_pos=reshape(prob_pos,dim(1),[]);
%    prob_ints=reshape(prob_ints,dim(1),dim(2),[]);
end
result=struct('o_mpll',o_mpll,'o_mpul',o_mpul,'o_mpmean',o_mpmean,'o_mpmedian',o_mpmedian,'prob_pos',prob_pos);
return